...
module.exports = {
    totalImpactAmount: async ctx => {
        try {
            let sumData = await strapi.connections.default.raw(`SELECT SUM(target_value) FROM impact_target_project where project = (${ctx.params.where.project}) and COALESCE(deleted, false) <> true`)
