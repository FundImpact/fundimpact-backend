  ...
  sdg_target_count: async (ctx) => {
    ...
    try {
      ...
      let queryString = `select sdg.id, sdg.name, sdg.icon , count(itp.id) from impact_category_org ico 
            ...
            } group by sdg.id ORDER BY count desc`;
  ...
  exportTable: async (ctx) => {
    try {
      ...
      const tableColumns = sendHeaderWhereValuesCanBeWritten
        ...
        : ["id", "name", "category", "target", "achieved", "progress", "sdg"];
      if (
      ...
      const impactTargetProjectStream = strapi.connections
        ...
        .join("impact_category_org", {
          [`impact_target_project.impact_category_org`]: "impact_category_org.id",
        })
  ...
  createImpactTargetProjectFromCsv: async (ctx) => {
    try {
      ...
      const validateRowToBeInserted = async (rowObj) =>
        await validateRowToBeInsertedInImpactTargetProject(
          rowObj,
...
const validateRowToBeInsertedInImpactTargetProject = async (
  rowObj,
  organizationId
) => {
  ...
  if (
    !(await isRowIdPresentInTable({
      rowId: rowObj.impact_category_org,
      strapi,
    ...
    return {
      valid: false,
      errorMessage: "impact_category_org not valid",
    };
  }
  if (
    !(await isRowIdPresentInTable({
      rowId: rowObj.impact_units_org,
      strapi,
    ...
    return {
      valid: false,
      errorMessage: "impact_units_org not valid",
    };
  }
