...
module.exports = {
    impact_achieved : async ctx => {
        try {
            ...
            where ico.organization = ${ctx.query.organization} group by projects.id) select id, name , ROUND((sum_itl * 100.0)/ sum_itp) as 
    ...
    exportTable : async ctx => {
        try{            
            await exportTableAsCsv({ctx, tableName: 'impact_target_project', whereCondition: {project: ctx.query.project_in[0]}})
        } catch(error) {
            console.log(error)
            return ctx.badRequest(null, error.message);
        }
