...
module.exports = {
  DeliverableSubTargetsConnection: async (ctx) => {
    try {
      let sumData = await strapi.connections.default
        .raw(`SELECT SUM(target_value),count(id) FROM deliverable_sub_targets 
        ...
        let obj = {
          deliverableSubTargetsConnection: {
            aggregate: {
              sum: {
                target_value:
                  sumData.rows[0].sum != null ? sumData.rows[0].sum : 0,
              },
