...
module.exports = {
    deliverable_achieved : async ctx => {
        try {
            ...
            where organization = ${ctx.query.organization} group by projects.id) select id, name , ROUND((sum_dtl * 100.0)/ sum_dtp) as avg_value from cte ORDER BY avg_value desc`)
