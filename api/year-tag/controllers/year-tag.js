...
module.exports = {
    donerWiseYearTag: async (ctx) => {
        const { id } = ctx.params;
        let data = await strapi.connections.default.raw(`SELECT DISTINCT yt.id,type,start_date,end_date FROM donors d 
    ...
    organizationWiseYearTag: async (ctx) => {
        const { id } = ctx.params;
        let data = await strapi.connections.default.raw(`SELECT DISTINCT yt.id,type,start_date,end_date FROM organizations org
